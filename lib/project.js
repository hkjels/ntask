...
Project.prototype.resolvePaths = function resolvePaths (path) {
  ...
  // This is needed before any other operations are
  // executed, hence the synchronous code.

  while (segments.length) {
    if (fs.existsSync(segments.join('/')+'/.todo')) {
...
Project.prototype.ignorePaths = function ignorePaths () {
  ...
  if (fs.existsSync(ignorefile)) {
    this.ignore = fs.readFileSync(ignorefile)
      .toString()
  ...
  // List of files that are ignored by default
