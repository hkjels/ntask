...
exports.find = function taskpoolFind (qargs, cb) {
  ...
  /**
   * FIXME @hkjels The task-filter should be refactored
   *     | I catch myself trying to understand this code
   *     | over and over again. ARGgegEGG!
   */
  ...
  tasks = tasks.filter(function (task) {
    ...
    if (qargs.id.length) {
      match = _.indexOf(qargs.id, task.__uuid__.substr(0, 8)) !== -1
    }

    // Fulltext search

    if (qargs.text.length) {
      match = (task.title+task.body).toLowerCase().indexOf(qargs.text) != -1
    }
